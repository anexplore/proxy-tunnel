...
public class MappingHolder {
    ...
    private final ConcurrentHashMap<String, TunnelMap> mapping;
    ...
    /**
     * find {@link TunnelMap} by local address
     *
     * @param host local address host
     * @param port local address port
     * @return {@link TunnelMap}
     */
    public TunnelMap resolveMappingByLocalAddress(String host, int port) {
     ...
     */
    public Set<Map.Entry<String, TunnelMap>> entrySet() {
        return mapping.entrySet();
    }

    /**
     * add not null port map to holder
     * @param portMap {@link TunnelMap} must not be null
     */
    public void addPortMap(TunnelMap portMap) {
     ...
     * use # to add comment
     * eg:
     *    # mapping local host 80 to local host 8080, port 80's any data will send to 8080
    ...
    public static MappingHolder buildMappingHolderFromFile(String filePath) throws Exception {
        ...
        for (String line : lines) {
            ...
            holder.addPortMap(new TunnelMap(new Address(localHostPort[0], Integer.parseInt(localHostPort[1])),
